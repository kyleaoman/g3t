/*
  ...
  print_affinity.c, Antonio Ragagnin <ragagnin@lrz.de> (c), 2019.
  ...
  Known issues:
      ...
      try to compile without -DSCHED flag (so the code will get the CPU id by parsing the output of '/proc/self/stat')
  ...
  Examples of compiling:
      ...
      OpenMP only:
          gcc -D_OPENMP -DSCHED -fopenmp print_affinity.c -o print_affinity.exe

      MPI + OpenMP:
          mpicc -D_MPI -D_OPENMP -DSCHED -fopenmp print_affinity.c -o print_affinity.exe

      MPI + OpenMP + OPENACC (with PGI compiler):
          mpicc -D_MPI -D_OPENMP -DSCHED -D_OPENACC -mp -acc -ta=tesla:cc70  print_affinity.c -o print_affinity.exe
